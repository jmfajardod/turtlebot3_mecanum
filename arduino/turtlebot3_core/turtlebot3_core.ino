...
void setup()
{
  ...
  motor_driver.init(NAME); // Name -> "Waffle Mecanum"
...
void loop()
{
  ...
  // Send goal velocities to motors
  ...
  // Publish relevant information (Sensors, Battery, drive)
  ...
  // If debud is enabled then publish debug data
...
{
  // Changed
  goal_velocity_from_cmd[LINEAR_X]  = cmd_vel_msg.linear.x; // Update value in pos 0
  goal_velocity_from_cmd[LINEAR_Y]  = cmd_vel_msg.linear.y; // Update value in pos 1
  goal_velocity_from_cmd[ANGULAR]   = cmd_vel_msg.angular.z;// Update value in pos 2
  goal_velocity_from_cmd[3]         = 0.0;                  // Update value in pos 3
...
void publishSensorStateMsg(void)
{
  ...
  sensor_state_msg.battery = sensors.checkVoltage(); // Voltage check

  dxl_comm_result = motor_driver.readEncoder(sensor_state_msg.left_front_encoder, sensor_state_msg.right_front_encoder, sensor_state_msg.left_rear_encoder, sensor_state_msg.right_rear_encoder); // DONE change to read encoder for 4 wheels

  if (dxl_comm_result == true)
    updateMotorInfo(sensor_state_msg.left_front_encoder, sensor_state_msg.right_front_encoder, sensor_state_msg.left_rear_encoder, sensor_state_msg.right_rear_encoder); // DONE dependant of result of line 275
  else
    return;

  sensor_state_msg.bumper = sensors.checkPushBumper(); // Check Bumpers

  sensor_state_msg.cliff = sensors.getIRsensorData();  // Check IR sensors

  sensor_state_msg.illumination = sensors.getIlluminationData(); // Check Illumination
  
  sensor_state_msg.button = sensors.checkPushButton(); // Check Push Button

  sensor_state_msg.torque = motor_driver.getTorque();  // DONE Check Torque

  sensor_state_pub.publish(&sensor_state_msg);         // Publish message
}
...
void publishVersionInfoMsg(void)
{
  ...
  version_info_msg.software = "0.0.0"; // Changed :v
...
void publishBatteryStateMsg(void)
{
  ...
  battery_state_msg.design_capacity = 1.8f; //Ah //TODO Change Battery Capacity
...
void publishDriveInformation(void)
{
  ...
  // calculate odometry
  calcOdometry((double)(step_time * 0.001)); // DONE change odometry calculation

  // odometry
  updateOdometry();  // DONE change update of odom
  ...
  // joint states
  updateJointStates(); // DONE change update of joint states
...
* Update the odometry
*******************************************************************************/
// CHANGED to incorporate linear movement in Y
...
void updateJointStates(void)
{
  ...
  joint_states_pos[LEFT_FRONT]  = last_rad[LEFT_FRONT]; // DONE Check update of last rad
  ...
  joint_states_vel[LEFT_FRONT]  = last_velocity[LEFT_FRONT]; // DONE Check update of last vel
...
*******************************************************************************/
// DONE change calc odometry

bool calcOdometry(double diff_time)
{
  float* orientation;
  double wheel_lf, wheel_rf, wheel_lr, wheel_rr;   // CHANGED   // rotation value of wheel [rad]
  double delta_x, delta_y, theta, delta_theta;
  static double last_theta = 0.0;
  double x_dot, y_dot, w;                  // v = translational velocity [m/s], w = rotational velocity [rad/s]
  double step_time;

  wheel_lf = wheel_rf = wheel_lr = wheel_rr = 0.0; // CHANGED wheels
  delta_x = delta_y = delta_theta = theta = 0.0; // CHANGED delta_s to delta_x, delta_y
  x_dot = y_dot = w = 0.0; // CHANGED  v to x_dot, y_dot
  step_time = 0.0;
  ...
  //************************************************************************************/
  // DONE CHANGE robot model 
  // Wheel 1 -> LEFT_FRONT
  // Wheel 2 -> RIGHT_FRONT 
  // Wheel 3 -> LEFT_REAR 
  // Wheel 4 -> RIGHT_REAR  
  
  delta_x     =  0.008*wheel_lf + 0.008*wheel_rf + 0.008*wheel_lr + 0.008*wheel_rr; // From Model
  delta_y     = -0.008*wheel_lf + 0.008*wheel_rf + 0.008*wheel_lr - 0.008*wheel_rr;  // From Model
   
  orientation = sensors.getOrientation();
  theta       = atan2f(orientation[1]*orientation[2] + orientation[0]*orientation[3], // Get YAW angle from IMU sensor
                0.5f - orientation[2]*orientation[2] - orientation[3]*orientation[3]);

  delta_theta = theta - last_theta; // Find delta
  
  // compute odometric pose 
  odom_pose[0] += delta_x * cos(odom_pose[2] + (delta_theta / 2.0)) - delta_y * sin(odom_pose[2] + (delta_theta / 2.0)) ; // From Model
  odom_pose[1] += delta_x * sin(odom_pose[2] + (delta_theta / 2.0)) + delta_y * cos(odom_pose[2] + (delta_theta / 2.0)) ; // From Model
  odom_pose[2] += delta_theta;

  // compute odometric instantaneous velocity
  // CHANGED added x_dot and y_dot
  
  x_dot = delta_x / step_time;
  y_dot = delta_y / step_time;
  ...
  // CHANGED from only 2 wheels to 4
...
*******************************************************************************/
// DONE Change behavior of drive using buttons

void driveTest(uint8_t buttons)
{
  static bool move[2] = {false, false};
  static int32_t saved_tick[4] = {0, 0, 0, 0}; // CHANGED from length 2 to 4
  static double diff_encoder = 0.0;

  int32_t current_tick[4] = {0, 0, 0, 0}; // CHANGED from length 2 to 4

  motor_driver.readEncoder(current_tick[LEFT_FRONT], current_tick[RIGHT_FRONT], current_tick[LEFT_REAR], current_tick[RIGHT_REAR]); // DONE change read encoder

  // CHECK for move LINEAR
  if (buttons & (1<<0))  
  {
    move[LINEAR_X] = true;
    ...
    diff_encoder = TEST_DISTANCE / (0.377 / 4096); // CHANGE circumference // (Circumference of Wheel) / (The number of tick per revolution)
    tTime[6] = millis();
  }
  else if (buttons & (1<<1))
  {
    ...
    diff_encoder = (TEST_RADIAN * TURNING_RADIUS) / (0.377 / 4096); // CHANGE circumference
    tTime[6] = millis();
  }
...
void updateVariable(bool isConnected)
{
  ...
  if (isConnected)
  {
    ...
    {      
      sensors.initIMU(); // Initialize IMU
      initOdom();        // Initialize Odom
...
void initOdom(void)
{
  ...
  odom.twist.twist.linear.y  = 0.0; // ADDED
...
{
  static char *joint_states_name[] = {(char*)"wheel_left_front_joint", (char*)"wheel_right_front_joint", (char*)"wheel_left_rear_joint", (char*)"wheel_right_rear_joint"}; // CHANGED from length 2 to 4
...
*******************************************************************************/
// DONE change update of goal velocity

void updateGoalVelocity(void)
{ 
  // ADDED goal velocity in Y
  ...
  sensors.setLedPattern(goal_velocity[LINEAR_X], goal_velocity[ANGULAR]); // DONE check what is led pattern
}
...
void sendDebuglog(void)
{
  ...
  int32_t encoder[WHEEL_NUM] = {0, 0, 0, 0}; // CHANGED from 2 to 4
  
  motor_driver.readEncoder(encoder[LEFT_FRONT], encoder[RIGHT_FRONT], encoder[LEFT_REAR], encoder[RIGHT_REAR]); // DONE change read encoder

  // CHANGED from 2 wheels to 4
